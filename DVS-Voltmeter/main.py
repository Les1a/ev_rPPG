...
def get_args_from_command_line():
    ...
    parser.add_argument('--model_para', type=float, nargs='+', help='Set parameters for a specific camera type',
                        default=None)
    parser.add_argument('--input_dir', type=str, help='Set dataset root_path',
                        default='../data/UBFC_img')
    parser.add_argument('--output_dir', type=str, help='Set output path',
                        default='../data/UBFC_ev')
    parser.add_argument('--process_model', type=int,
                        help='0:process all the dirs in input path dir; 1:process the input path dir', default=0)
...
def process_dir(cfg, file_info, video_name):
    ...
    # np.savetxt(os.path.join(cfg.DIR.OUT_PATH, video_name, video_name + '.txt'), events, fmt='%1.0f')
    np.savetxt(os.path.join(cfg.DIR.OUT_PATH, video_name + '.txt'), events, fmt='%1.0f')
    sim.reset()


def process_dir_remain(cfg, file_info, video_name):
    indir = os.path.join(cfg.DIR.IN_PATH, video_name)
    outdir = os.path.join(cfg.DIR.OUT_PATH, video_name)
    print(f"Processing folder {indir}... Generating events in file {outdir}")
...
if __name__ == "__main__":
    ...
    if args.process_model == 0:
        ''' multiple (simulating all dirs refer to path) '''
        ...
        ''' single (splitting a long sequence to several short ones) '''
        os.makedirs(cfg.DIR.OUT_PATH, exist_ok=True)
        video_path = cfg.DIR.IN_PATH
        if is_valid_dir(video_path):
            # video info
            with open(os.path.join(video_path, 'info.txt'), 'r') as f:
                video_info = f.readlines()
            # simulation
            length = len(video_info)
            spilt_num = 480
            sequence_num = int(length / spilt_num)
            for i in range(sequence_num):
                print(f'Processing seg {i:03d}')
                if i != sequence_num - 1:
                    process_dir_remain(cfg=cfg, file_info=video_info[spilt_num * i:spilt_num * (i + 1)],
                                       video_name=f'ev_rs_{(i + 1):03d}')
                else:
                    process_dir(cfg=cfg, file_info=video_info[spilt_num * i:spilt_num * (i + 1)],
                                video_name=f'ev_rs_{(i + 1):03d}')
